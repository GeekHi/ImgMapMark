	...
	$.fn.ZoomMark = function (method) {
		...
		this.publicMethods = {
			init: function (options) {
				...
				mContainer.mousedown(function (event) {
					if (event.which == 1) {
						...
						} else if(event.target.className.indexOf("markCodeShow")!=-1){
							var IDName = event.target.id;
							var IdNumber = IDName.split("_")[1];
							mContainer.moveMark = IdNumber;
							rotateId = IdNumber;
						...
						} else if (event.target.className == "operDelete") {
							mContainer.publicMethods.deleteMark(operId);
						} else if (event.target.className == "operEdit") {
			...
			mark: function (config) {
				...
				mContainer.append($('<div class="icon-'+config.type+'" id="mark_' + newMark.id + '"><span class="markCodeShow" id="markcode_'+ newMark.id +'">'+config.markCode+'</span></div>'));
