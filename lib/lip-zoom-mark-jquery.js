		...
		this.publicMethods = {
			init: function (options) {
				...
				document.oncontextmenu = function (event) {
					if (event.target.className.indexOf("icon-")!=-1 || event.target.className.indexOf("markCodeShow")!=-1 ) {
						var poX = event.pageX - mContainer.offset().left;
						var poY = event.pageY - mContainer.offset().top;
						$(".operMenu").show();
						operId = event.target.id.split("_")[1];
						$(".operMenu").css("left", poX);
						$(".operMenu").css("top", poY);
					}
					return false;
				}
				mContainer.append($('<ul class="operMenu"><li class="operView">查看</li><li class="operDelete">删除</li><li class="operEdit">编辑</li></ul>'));
				...
				mContainer.mousedown(function (event) {
					if (event.which == 1) {
						...
						// 点击选中拖拽坐标
						if (event.target.className.indexOf("icon-")!=-1 && switchKey) {
							var IDName = event.target.id;
							var IdNumber = IDName.split("_")[1];
							mContainer.moveMark = IdNumber;
							rotateId = IdNumber;
							if(IdNumber!="temp"){
								$("#rotate").show();
								$("#extention").show();
							}
						} else if(event.target.className.indexOf("markCodeShow")!=-1 && switchKey){
							var IDName = event.target.id;
							var IdNumber = IDName.split("_")[1];
							mContainer.moveMark = IdNumber;
							rotateId = IdNumber;
						...
						else if(event.target.className=="operView"){
							top.openWindow($("#markcode_"+operId).text());
						} else if (event.target.className == "operDelete" && switchKey) {
						    top.deleteMarkAsset({
								aid:$("#markcode_"+operId).text(),
								mid:operId
							})
							// mContainer.publicMethods.deleteMark(operId);
						} else if (event.target.className == "operEdit" && switchKey ) {

							// top.showMarkCodeInput(operId);
							 top.showMarkCodeSelect(operId);
						} else {
							...
							if(event.target.className!='lipButton'){
								$("#rotate").hide();
								$("#extention").hide();
							}
						}
				...
				mContainer.mousemove(function (event) {
					// console.log(mousePosition.isMouseDown);
					...
					if (mContainer.moveMark != null && switchKey) {
						var poX = event.pageX - mContainer.offset().left; //获取当前鼠标相对img的X坐标  
						var poY = event.pageY - mContainer.offset().top; //获取当前鼠标相对img的Y坐标 

						$("#mark_" + mContainer.moveMark).css("left", poX);
						$("#mark_" + mContainer.moveMark).css("top", poY);

						var data = mContainer.data('ZoomMarkData');
						// console.log(data);
						data.mMarks[mContainer.moveMark - 1].x = poX;
						data.mMarks[mContainer.moveMark - 1].y = poY;
						mContainer.data('ZoomMarkData', data);

					}

					if (markIndex != null && switchKey) {
						$("#mark_temp").css("display", "block");
						var poX = event.pageX - mContainer.offset().left; //获取当前鼠标相对img的X坐标  
						var poY = event.pageY - mContainer.offset().top; //获取当前鼠标相对img的Y坐标 

						$("#mark_temp").css("left", poX);
						$("#mark_temp").css("top", poY);
					}

				});
				mContainer.mouseup(function (event) {
					mousePosition.isMouseDown = false;
					mContainer.moveMark = null;
					event.preventDefault();

					if (markIndex && switchKey) {
						$("#mark_temp").remove();
						mContainer.publicMethods.mark({
							x:event.pageX - mContainer.offset().left, 
							y:event.pageY - mContainer.offset().top,
							type:markIndex,
							markCode:""
						});
						// settings.afterMark(mMarks);
						// top.showMarkCodeInput(mMarks.length);
						top.showMarkCodeSelect(mMarks.length);
					}
			...
			mark: function (config) {
				...
				if(config.status=="在租"){
					showColor = "#f65732";
				} else if (config.status=="待租"){
					showColor = "#2ad63e";
				} else if (config.status=="不可用"){
					showColor = "#3b32f6"; 
				} else {
					showColor = "#000"; 
				}
				...
				mContainer.append($('<div class="icon-'+ Number(config.type) +'" id="mark_' + newMark.id + '"><span class="markCodeShow" style="color:'+showColor+'"  id="markcode_'+ newMark.id +'">'+config.markCode+'</span></div>'));

				var object = $('#mark_' + mMarks.length);
				object.css('left', config.x);
				object.css('top', config.y);
				object.css("width",config.w);
				object.css({ 'transform': 'rotate(' + config.r + 'deg)' });
				data.mMarks = mMarks;
				mContainer.data('ZoomMarkData', data);
				return mMarks;
			},
			...
			changeSettings: function (options) {
				...
				if (options.imgPosition) {
					data.imgPosition = options.imgPosition;
				}
				mContainer.data('ZoomMarkData', data);
			...
			// 重新设置已标记图标markCode
			setMarkCode:function(id,code){
				if (!id)
				return;
				var data = mContainer.data('ZoomMarkData');
				for (var i = 0; i < data.mMarks.length; i++) {
					if (data.mMarks[i].id == id) {
						data.mMarks[i].markCode = code;
					}
				}
			},
			// 标记角度赋值
			setMarkRotate:function(id,deg){
				if (!id)
				return;
				var data = mContainer.data('ZoomMarkData');
				for (var i = 0; i < data.mMarks.length; i++) {
			...
			setMarkList: function (list) {
				...
				$.each(marks,function(index,item){
					mContainer.publicMethods.deleteMark(item.id);
				})
				data.mMarks = [];
				mContainer.data('ZoomMarkData', data);
			...
			setAddState: function (flag) {
				markIndex = flag;
				if(switchKey){
					mContainer.append($('<div class="icon-'+Number(markIndex) +'" id="mark_temp" style="display:none"></div>'));
				}
			},
			getRotateId:function(){
				return rotateId;
			}
		}
